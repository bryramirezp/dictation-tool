...
(function () {
    ...
    if (!root)
        return;
    var shotsGroup = root.querySelector("[data-demo-shots]");
    var anim = root.querySelector("[data-demo-anim]");
    var logEl = root.querySelector("[data-demo-log]");
    var barsEl = root.querySelector("[data-demo-bars]");
    var statusEl = root.querySelector("[data-demo-status]");
    var captionEl = root.querySelector("[data-demo-caption]");
    if (!shotsGroup || !anim || !logEl || !barsEl || !statusEl || !captionEl)
        return;
    ...
    fetch("demo-data.json")
        .then(function (res) {
        if (!res.ok)
            throw new Error("no demo-data.json yet");
        return res.json();
    })
        .then(function (data) {
        if (reduceQuery.matches)
        ...
        function listenPhase() {
            if (stopped || reduceQuery.matches) {
                revertToStatic();
                return;
            }
        ...
        function endListening() {
            ...
            logEl.insertAdjacentHTML("beforeend", '<p class="win-fake-dim">Recorded ' + seconds + "s</p>");
